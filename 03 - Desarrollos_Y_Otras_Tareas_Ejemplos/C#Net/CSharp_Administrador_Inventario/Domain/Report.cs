	...
	public class Report
	{
		...
		public void ExportTransactionLog( DataGridView dataGridView1, string ReportTitle)
		{
			...
			if(saveFile.ShowDialog() == DialogResult.OK)
			{
				if(IsFileInUse(saveFile.FileName))
			     	{
			     		MessageBox.Show("El arcivo está siendo usado por otra aplicación", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
			     		return;
			     	}
				...
				{	
					...
					doc.Open();
						...
						{
					  		MessageBox.Show("Sin datos para exportar.");
							return;
						}
		...
		public void ExportReceivableLog( DataGridView dataGridView1, string ReportTitle)
		{
			...
			if(saveFile.ShowDialog() == DialogResult.OK)
			{
				if(IsFileInUse(saveFile.FileName))
			     	{
			     		MessageBox.Show("El archivo está siendo utilizado por otra aplicación.", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
			     		return;
			     	}
					Document doc = new Document(PageSize.LETTER_LANDSCAPE,10, 10, 42, 30);
					Paragraph para = new Paragraph("VTDI-MAN-TRANSACTION-LOG");
			     ...
			     if(saveFile.ShowDialog() == DialogResult.OK)
					{
			     	if(IsFileInUse(saveFile.FileName))
			     	{
			     		MessageBox.Show("El archivo está en uso por otro programa.","", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
			     		return;
			     	}
        ...
        public void ExportRequest(DataGridView dataGridView1, string ReportTitle)
        {
            ...
            if (saveFile.ShowDialog() == DialogResult.OK)
            {
                if (IsFileInUse(saveFile.FileName))
                {
                    MessageBox.Show("El archivo está siendo usado por otra aplicación.", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                    return;
                }
                Document doc = new Document(PageSize.LETTER_LANDSCAPE, 10, 10, 42, 30);
                ...
                try
                {
                    ...
                    if (dataGridView1.Columns.Count == 0)
                    {
                        MessageBox.Show("No existen datos para exportar.");
                        return;
                    }
                    PdfPTable table = new PdfPTable(dataGridView1.Columns.Count);
