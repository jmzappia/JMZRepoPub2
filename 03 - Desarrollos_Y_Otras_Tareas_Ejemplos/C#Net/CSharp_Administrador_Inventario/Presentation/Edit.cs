	...
	public partial class Edit : Form
	{
		...
		void SaveBasicInfo()
		{
			...
			if(IsInfoEmpty())
			{
			 ...
			 errLb2.Visible = false;
	    	   ...
	    	   DialogResult dialog =  MessageBox.Show("¿Está seguro de guardar los cambios? ", "", MessageBoxButtons.YesNo);
	    	   ...
	    	   try
	    	   {
                using (OleDbConnection accessConnection = new OleDbConnection(Connection.getConnectionString()))
                {
				 ...
				 {
				 	 MessageBox.Show("Error inesperado. No se puede actualizar.\n" + ex, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error );	
				 }
				 finally
				 {
		...
		void SaveQuantityInfo()
		{
			...
			errlbl1.Visible = false;
				...
				{
					MessageBox.Show("Por favor, seleccione una unidad válida", "Invalid", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
					return;
				}
	    	    newItem.ItemDescription = itemDescriptionTxtBox.Text;
			    DialogResult dialog =  MessageBox.Show("¿Desea guardar los cambios? ", "", MessageBoxButtons.YesNo);
				  if(dialog == DialogResult.Yes)
				  {
						   ...
						   else
						   {
						   	   try
					    	   {
                                    using (OleDbConnection accessConnection = new OleDbConnection(Connection.getConnectionString()))
                                    {
                                        ...
                                        cmd.ExecuteNonQuery();
                                        MessageBox.Show("Registro actualizado correctamente.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);  //inform the user
                                        Data_Access.DataAccess.LogAction(userId, "Updated Quantity for item: " + ItemlistBox.SelectedItem);
                                        Clear();
                                        if (!(ItemlistBox.Items.Count <= 0))
                                        {
								 ...
								 {
                                    MessageBox.Show("Inténtelo de nuevo.", "Connection Issue", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                 }
                                 finally
								 {
								 	refreshList();
		...
		void refreshList()
		{
			...
			{
                MessageBox.Show("Imposible cargar los registros."+e.Message + e.StackTrace, "Connection Issue", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            finally
			{
			}
		...
		void SaveBasicBtnClick(object sender, EventArgs e)
		{
			if(ItemlistBox.SelectedIndex < 0)
			{
				MessageBox.Show("No ha seleccionado un ítem");
				return;
			}
			SaveBasicInfo();
		...
		void SaveQuantityBtnClick(object sender, EventArgs e)
		{
			if(ItemlistBox.SelectedIndex < 0)
			{
				MessageBox.Show("No ha seleccionado un ítem", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
				return;
			}
			SaveQuantityInfo();
		}
